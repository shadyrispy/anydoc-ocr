#!/usr/bin/env bash
# 单文件自解压安装器构建（方案 C，多架构）
#
# 用法: scripts/package-single.sh [aarch64|x86_64]
#   aarch64 (默认) → dist/anydoc-ocr-linux-arm64.run
#   x86_64          → dist/anydoc-ocr-linux-x86_64.run
#
# 产物 = [自解压壳 bash 脚本][tar.gz 数据段]，目标机一键部署：
#   首次运行解压到 $ANYDOC_INSTALL_DIR（默认 ~/.anydoc-ocr），
#   自动把包内 CJK 字体装入用户字体目录（fontdb load_system_fonts 生效），
#   exec run.sh（设 OAR_HOME 指向包内模型 / LD_LIBRARY_PATH 指向包内 lib）。
#   之后直跑免解压；--reinstall 重装。
#
# 内嵌资源：bin + libonnxruntime.so + libpdfium.so + NotoSansCJK.ttc
#           + tiny 模型 8 件（OAR_HOME 布局）。
# small/medium 模型不内嵌，运行期 ANYDOC_MODEL_DIR 外置（README）。
set -euo pipefail
cd "$(dirname "$0")/.."

ARCH="${1:-aarch64}"
case "$ARCH" in
  aarch64)
    TARGET=aarch64-unknown-linux-gnu
    OUT="dist/anydoc-ocr-linux-arm64.run"
    PKG_DIR="anydoc-ocr-linux-arm64"
    ORT_SO="third_party/ort/aarch64/onnxruntime-linux-aarch64-1.20.1/lib/libonnxruntime.so"
    PDFIUM_SO="third_party/pdfium/aarch64/lib/libpdfium.so"
    ;;
  x86_64|x64)
    TARGET=x86_64-unknown-linux-gnu
    OUT="dist/anydoc-ocr-linux-x86_64.run"
    PKG_DIR="anydoc-ocr-linux-x86_64"
    ORT_SO="third_party/ort/x64/onnxruntime-linux-x64-1.20.1/lib/libonnxruntime.so"
    PDFIUM_SO="third_party/pdfium/x64/lib/libpdfium.so"
    # x64 为本机构建（host，无 triple 前缀目录）
    BIN="target/release/anydoc-ocr"
    ;;
  *) echo "未知架构: $ARCH（支持 aarch64 / x86_64）"; exit 1 ;;
esac

VERSION=$(grep '^version' Cargo.toml | head -1 | cut -d'"' -f2)
STAGE="dist/$PKG_DIR"
SELF_SIZE_SENTINEL="000000000000"   # 12 位定宽占位，替换后字节数不变

# 0. 前置校验
[ -f "$ORT_SO" ] || { echo "缺少 $ORT_SO（先按架构放置预编译库）"; exit 1; }
[ -f "$PDFIUM_SO" ] || { echo "缺少 $PDFIUM_SO"; exit 1; }
[ -f fonts/NotoSansCJK-Regular.ttc ] || { echo "缺少 fonts/NotoSansCJK-Regular.ttc（先跑 scripts/install-font.sh）"; exit 1; }
# x64 已在分支内定 BIN（host 无 triple 前缀目录）；aarch64 用 triple 目录
BIN="${BIN:-target/$TARGET/release/anydoc-ocr}"
[ -f "$BIN" ] || { echo "缺少 $BIN（先交叉/本机构建 release）"; exit 1; }

# 1. 组装目录包
rm -rf "$STAGE" && mkdir -p "$STAGE/lib" "$STAGE/oar-home" "$STAGE/fonts"
cp "$BIN" "$STAGE/"
# 复制 .so* 全部变体（含多版本链接：libonnxruntime.so → .so.1 → .so.1.20.1；
# 只 cp 链接本身会断链，运行时找不到 .so.1）
cp "$ORT_SO"* "$STAGE/lib/" 2>/dev/null || true
cp "$PDFIUM_SO"* "$STAGE/lib/" 2>/dev/null || true
cp fonts/* "$STAGE/fonts/" 2>/dev/null || true

# 离线模型：只内嵌 tiny 档 8 件（复用 auto-download 缓存布局：扁平 + .sha256 伴生）。
# small/medium 不内嵌（全档 348M 爆炸），运行期 ANYDOC_MODEL_DIR 外置。
TINY_MODELS="pp-doclayout-s.onnx picodet_layout_1x_table.onnx \
pp-ocrv6_tiny_det.onnx pp-ocrv6_tiny_rec.onnx ppocrv6_tiny_dict.txt \
slanet_plus.onnx pp-lcnet_x1_0_table_cls.onnx table_structure_dict_ch.txt"
PACKED=0
for m in $TINY_MODELS; do
  if [ -f "$HOME/.oar/$m" ]; then
    cp "$HOME/.oar/$m" "$STAGE/oar-home/"
    [ -f "$HOME/.oar/$m.sha256" ] && cp "$HOME/.oar/$m.sha256" "$STAGE/oar-home/"
    PACKED=$((PACKED + 1))
  fi
done
if [ "$PACKED" -eq 0 ]; then
  echo "警告: $HOME/.oar 无 tiny 模型，离线包无模型（将走在线 auto-download）"
fi

# 启动器：一键部署——OAR_HOME/LD_LIBRARY_PATH 指向包内；CJK 字体装入用户字体
# 目录（ofd-core 的 fontdb 走 load_system_fonts，不扫包内，必须装进
# $XDG_DATA_HOME/fonts 或 ~/.fonts 才生效；用户级安装免 root、幂等）。
cat > "$STAGE/run.sh" <<'EOF'
#!/usr/bin/env bash
DIR="$(cd "$(dirname "$0")" && pwd)"
export OAR_HOME="${OAR_HOME:-$DIR/oar-home}"
export LD_LIBRARY_PATH="$DIR/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
# 一键字体：包内 CJK 字体 → 用户字体目录（fontdb load_system_fonts 扫描）
if [ -f "$DIR/fonts/NotoSansCJK-Regular.ttc" ]; then
  FD="${XDG_DATA_HOME:-$HOME/.local/share}/fonts"
  mkdir -p "$FD"
  TARGET_FONT="$FD/anydoc-ocr-NotoSansCJK-Regular.ttc"
  if [ ! -f "$TARGET_FONT" ]; then
    cp "$DIR/fonts/NotoSansCJK-Regular.ttc" "$TARGET_FONT"
    echo "[anydoc-ocr] 已安装 CJK 字体到 $TARGET_FONT"
  fi
fi
exec "$DIR/anydoc-ocr" "$@"
EOF
chmod +x "$STAGE/run.sh"

# 2. 打 tar.gz（-C dist 保留顶层目录，strip-components=1 解压展平）
TARBALL=$(mktemp)
trap 'rm -f "$TARBALL"' EXIT
tar czf "$TARBALL" -C dist "$PKG_DIR"

# 3. 生成自解压壳（OFFSET 占位 12 位，构建时注入真实偏移）
SHELL_TMP=$(mktemp)
cat > "$SHELL_TMP" <<'SHELL_EOF'
#!/usr/bin/env bash
# anydoc-ocr 单文件安装器（自解压，一键部署）。
# 用法: ./anydoc-ocr-linux-*.run [--reinstall] [anydoc-ocr 参数...]
set -euo pipefail
DIR="${ANYDOC_INSTALL_DIR:-$HOME/.anydoc-ocr}"
OFFSET=000000000000
if [ "${1:-}" = "--reinstall" ]; then
  rm -rf "$DIR"
  shift
fi
if [ ! -x "$DIR/anydoc-ocr" ]; then
  mkdir -p "$DIR"
  # 解压尾部 tar.gz 段（OFFSET 为数据段起始，1-based）；strip-components=1
  # 去掉包内顶层目录，使 run.sh/lib/fonts 平铺于 $DIR
  tail -c +$OFFSET "$0" | tar xz --strip-components=1 -C "$DIR"
  echo "已安装 anydoc-ocr 到 $DIR（之后直接运行本文件即可；--reinstall 重装）"
fi
exec "$DIR/run.sh" "$@"
exit 0
SHELL_EOF

SHELL_SIZE=$(wc -c < "$SHELL_TMP")
OFFSET=$((SHELL_SIZE + 1))   # tar.gz 紧跟壳最后一行换行符之后（1-based）
sed "s/OFFSET=${SELF_SIZE_SENTINEL}/OFFSET=$(printf '%012d' "$OFFSET")/" "$SHELL_TMP" > "$OUT"
cat "$TARBALL" >> "$OUT"
chmod +x "$OUT"
rm -f "$SHELL_TMP"

echo "单文件安装器: $OUT ($(du -h "$OUT" | cut -f1))"
echo "目标机一键部署: ./$(basename "$OUT") [--reinstall] [参数...]"
